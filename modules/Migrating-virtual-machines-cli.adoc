...
* Plan: Specifies the source and target clusters, network mapping, data store mapping, and VMs to migrate
* Migration: Runs the migration plan
...
. Create a Secret with the VMware credentials:
+
[source,terminal]
----
$ cat < EOF | oc apply -f -
...
spec:
  ...
  secret:
    name: vmware-provider-secret <2>
    namespace: openshift-migration
EOF
----
<1> Specify the vSphere API URL.
...
. Create a Migration CR to run the migration plan:
+
[source,terminal]
----
$ cat < EOF | oc apply -f -
