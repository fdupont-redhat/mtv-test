...
. Create an operator group for the `openshift-migration` namespace:
+
[source,terminal]
----
$ cat << EOF | oc apply -f -
...
. Create a `virt-operator` subscription:
+
[source,terminal]
----
$ cat << EOF | oc apply -f -
...
. Create a `virt-controller` custom resource:
+
[source,terminal]
----
$ cat << EOF | oc apply -f -
...
. View the resources created by the `virt-controller`:
+
[source,terminal]
----
