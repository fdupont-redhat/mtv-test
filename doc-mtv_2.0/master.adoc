...
{product-first} enables you to migrate a large number of virtual machines from VMware vSphere to {virt}.

You can download the VMware provider data collected by {product-short} and upload it to the Migration Analytics service on link:https://cloud.redhat.com/migrations/migration-analytics[cloud.redhat.com] for a detailed workload analysis report and migration recommendations.
...
== Installing {product-full}

You can install {product-first} by using the {virt} web console or the CLI.

Then you can create a VMware Virtual Disk Development Kit (VDDK) image. {product-short} uses VDDK to copy the source disks.
