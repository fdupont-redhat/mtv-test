...
# this.
if [[ "$BRANCH" =~ ^main$|^[0-9]+\.[0-9]+\.X$ ]]; then
    echo "Branch is main, so pushing docs to gh-pages"
    git add --all
    git commit -am '[ci skip] publishing updated documentation...'

    git remote rm origin
    git remote add origin https://$GH_NAME:$GH_TOKEN@github.com/apinnick/mtv-test.git

    git push origin gh-pages
else
    echo "Not on main, so won't push doc"
fi
